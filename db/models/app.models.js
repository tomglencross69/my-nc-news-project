...
exports.fetchArticleById = (article_id) => {
    return db
    .query(`SELECT articles.article_id, articles.title, articles.topic, articles.author, articles.created_at, articles.votes, articles.article_img_url, articles.body, COUNT(comments.comment_id) AS comment_count
    FROM articles
    LEFT JOIN comments
...
exports.fetchUserByUsername = (username) => {
    if (!isNaN(username)) {
        return Promise.reject({status: 400, msg: 'Bad request: invalid username format'})
    }
    return db
    .query(`SELECT * FROM users
        ...
        .then(({rows}) => {
            if (!rows.length) {
                return Promise.reject({status: 404, msg: 'User not available or does not exist'})
            }
            return rows[0]
        }) 
}